...
export class LessonsService {
  ...
  remove(id: number) {
    ...
    return this.prisma.$transaction(async (prisma) => {
      // check existence
      const existing = await prisma.lecciones.findUnique({ where: { id_Leccion: id } });
      if (!existing) {
      ...
      const retos = await prisma.retos.findMany({ where: { Lecciones_id_Leccion: id } });

      // delete comandos for each reto
      const retoIds = retos.map((r) => r.id_Reto);
      if (retoIds.length) {
        await prisma.comandos.deleteMany({ where: { Retos_id_Reto: { in: retoIds } } });
      }
      ...
      // delete leccion
      const deleted = await prisma.lecciones.delete({ where: { id_Leccion: id } });
      return deleted;
    });
  }
}
